...
char uwb_zyl::openUwblpsSerial(const char *port, int baud, char bits, parity parity, char stopbit)
{
...
//已经检查开头的0X55和0X01只需要读取后面的126位即可
{
    ...
    if(recieve_buf_sum == recieve_buf[128-1])
    {
        ...
        log<<this->count<<", "<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<
                        i_uwb_lps.position.z<<" "<<i_uwb_lps.dis_buf[0]<<","<<i_uwb_lps.dis_buf[1]<<","<<i_uwb_lps.dis_buf[2]<<","
           <<i_uwb_lps.dis_buf[3]<<","<<i_uwb_lps.dis_buf[4]<<","<<i_uwb_lps.dis_buf[5]<<","<<i_uwb_lps.dis_buf[6]<<","
                <<i_uwb_lps.dis_buf[7]<<endl;
...
Point2D uwb_zyl::uwblps_Data_Convert(const uint8 *recieve_buf, int p)
{
    ...
    if((recieve_buf[0 + p] == 0x55)&&(recieve_buf[1 + p] == 0x01))
    {
        ...
        if(recieve_buf_sum == recieve_buf[127+p])
        {
            ...
            res.x = i_uwb_lps.position.x;
            res.y = i_uwb_lps.position.y;
            cout<<"good: "<<this->count<<": ("<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<i_uwb_lps.position.z<<")\n";
            log<<"good: "<<this->count<<": ("<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<
               i_uwb_lps.position.z<<")\n";
            //log<<"原始距离： "<<this->count<<": ("<<<<","<<tempdis.dis2<<","<<tempdis.dis3<<","<<tempdis.dis4<<")\n";
...
Point2D uwb_zyl::uwblps_Data_Convert(const uint8 *recieve_buf, int p,const string filename)
{
    ofstream the_file;
    the_file.open("./"+filename,ios::app);
    uint8 recieve_buf_sum = 0;
    //  uint8 i = 0;
    ...
    if((recieve_buf[0 + p] == 0x55)&&(recieve_buf[1 + p] == 0x01))
    {
        ...
        if(recieve_buf_sum == recieve_buf[127+p])
        {
            ...
            // cout<<"good: "<<this->count<<": ("<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<i_uwb_lps.position.z<<")\n";
            //log<<"good: "<<this->count<<": ("<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<i_uwb_lps.position.z<<")\n";
            the_file<<"good: "<<this->count<<": ("<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<
                    i_uwb_lps.position.z<<")\n";
            //log<<"原始距离： "<<this->count<<": ("<<<<","<<tempdis.dis2<<","<<tempdis.dis3<<","<<tempdis.dis4<<")\n";
...
Point2D uwb_zyl::uwblps_Data_Convert2(const uint8 *recieve_buf, int p,const string filename)
{
    ofstream this_log;
    this_log.open("../data/"+filename,ios::app);
    ...
    if((recieve_buf[0 + p] == 0x55)&&(recieve_buf[1 + p] == 0x01))
    {
        ...
        if(recieve_buf_sum == recieve_buf[127+p])
        {
            ...
            this_log<<"good: "<<this->count<<": ("<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<
               i_uwb_lps.position.z<<")\n";
            this_log<<this->count<<", "<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<<
               i_uwb_lps.position.z<<" "<<i_uwb_lps.dis_buf[0]<<","<<i_uwb_lps.dis_buf[1]<<","<<i_uwb_lps.dis_buf[2]<<","
               <<i_uwb_lps.dis_buf[3]<<","<<i_uwb_lps.dis_buf[4]<<","<<i_uwb_lps.dis_buf[5]<<","<<i_uwb_lps.dis_buf[6]<<","
               <<i_uwb_lps.dis_buf[7]<<endl;
            //cout<<"good: "<<this->count<<": ("<<i_uwb_lps.position.x<<","<<i_uwb_lps.position.y<<","<< i_uwb_lps.position.z<<")\n";
...
void uwb_zyl::getUwbPosition(Point2D &the_point, const string filename)
{
    uint8 buffer[512] = {0};
    while(1)
    {
        memset(buffer,0,512);
        int rd_num = this->readData(buffer);
        //cout<<rd_num<<endl;
        if(rd_num)
        {
            for(int i = 0;i<rd_num;i++)
            {
                if(buffer[i] == 0x55 && buffer[i+1] == 0x01)
                {
                    ...
                    if(rd_num - i >127)
                    {
                        the_point = this->uwblps_Data_Convert2(buffer,i,filename);
                    }
                }
            }
        }
